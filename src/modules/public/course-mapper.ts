...
/** Explorer `Course` card shape (matches `edupath-explorer/src/lib/data.ts`) */
export type ExplorerCourseDto = {
  ...
  cover: string;
  description: string;
  videos: { id: string; title: string; duration: string; locked?: boolean }[];
};
...
function resolveCoverUrl(course: Course | Record<string, any>, mediaBase?: string): string {
  ...
  const raw = candidates.find((x) => typeof x === 'string' && String(x).trim().length > 0) as string | undefined;
  const u = (raw || '').trim();
  if (!u) return DEFAULT_COVER;
  if (u.startsWith('//')) return `https:${u}`;
  if (u.startsWith('http://') || u.startsWith('https://')) return u;
  if (u.startsWith('/') && mediaBase) {
    return `${mediaBase.replace(/\/$/, '')}${u}`;
  }
  if (u.startsWith('/')) return DEFAULT_COVER;
  return u;
}
...
function flattenLessons(modules: any): Array<{
  title: string;
  durationSec: number;
  freePreview?: boolean;
}> {
  const out: Array<{ title: string; durationSec: number; freePreview?: boolean }> = [];
...
export function mapCourseToExplorerDto(
  ...
  const lessonCount =
    course.lessonCount > 0 ? course.lessonCount : lessons.length || Math.max(1, (course as any).videos?.length || 0);
